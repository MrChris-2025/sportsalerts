...
function renderScoreboard(events) {
  ...
  scoreboard.innerHTML = ''; // Clear prior nodes
  ...
  events.forEach(game => {
    ...
    card.innerHTML = `
      <div class="matchup">${game.shortName}</div>
      <div class="score-display">
        <span>${team2.team.abbreviation} ${team2.score || '0'}</span>
        <span> - </span>
        <span>${team1.team.abbreviation} ${team1.score || '0'}</span>
      </div>
      <div class="game-status">${game.status.type.detail}</div>
      <button class="btn-subscribe" onclick="window.subscribeToGame('${game.id}')" ${game.status.type.name === 'STATUS_FINAL' ? 'disabled' : ''}>
        ${game.status.type.name === 'STATUS_FINAL' ? 'Game Ended' : 'Get Live Push Notifications'}
      </button>
    `;
...
window.subscribeToGame = async function(gameId) {
  ...
  try {
    ...
    await Parse.Cloud.run("trackGame", { gameId: gameId });
